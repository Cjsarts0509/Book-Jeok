...
const Manual = (() => {
  ...
  const adminSections = `
    <section class="manual-sec">
      <h2>1. 대시보드</h2>
      <ul>
        <li><b>🏠 대시보드</b> 탭 — 계정/파일 수, 디스크 사용·할당 현황, 바이러스 검사 상태를 한눈에 봅니다.</li>
    ...
    <section class="manual-sec manual-tips">
      ...
      <ul>
        ...
        <li><b>바이러스 검사</b> — ClamAV(clamd)를 연결하면 업로드·업로드요청 파일을 자동 검사해 감염 파일을 차단합니다(대시보드에서 상태 확인). 미연결 시에는 검사를 건너뜁니다.</li>
      </ul>
    </section>`;
