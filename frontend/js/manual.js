...
const Manual = (() => {
  ...
  const adminSections = `
    <section class="manual-sec">
      <h2>1. 대시보드</h2>
      <ul>
        <li><b>🏠 대시보드</b> 탭 — 계정/파일 수, 디스크 사용·할당 현황, 파일형식 검증·악성패턴(YARA) 상태를 한눈에 봅니다.</li>
    ...
    <section class="manual-sec">
      <h2>5. 공유 통합 관리</h2>
      <ul>
        <li><b>🔗 공유</b> 탭 — <b>모든 계정</b>의 외부 공유를 한 곳에서 봅니다: 파일·압축 공유, 폴더 공유(읽기 전용), 업로드 요청.</li>
        <li>각 항목에 <b>대상 계정·비밀번호 여부·만료·다운로드/조회/수신량</b>이 표시되며, <span class="kbd">📋</span>로 링크 복사, <b>폐기</b>로 즉시 차단할 수 있습니다.</li>
      </ul>
    </section>

    <section class="manual-sec">
      <h2>6. 휴지통 · 용량(트리맵) · 리포트 · DB · 로그</h2>
      <ul>
        <li><b>휴지통</b> — 전 계정의 삭제 항목을 <b>복원/영구삭제</b>. 1년 지난 항목은 자동 영구삭제(사용자는 본인 것 30일 셀프복원).</li>
        <li><b>용량(트리맵)</b> — <span class="kbd">📊 용량</span> 탭은 <b>SpaceSniffer식 트리맵</b>입니다. 타일 크기 = 사용량, 색상 = 할당률(파랑→빨강). <b>계정 → 최상위 폴더 → 그 하위 폴더</b> 순으로 <b>한 단계씩 파고들며</b>(📁 하위 있음 / 📂 하위 없음 / 📄 이 폴더의 파일), 상단 <b>브레드크럼</b>(전체 계정 / 계정 / 폴더…)에서 원하는 단계로 바로 돌아갑니다. 마우스를 올리면 상세가 뜹니다.</li>
        <li><b>주간 리포트</b> — <span class="kbd">📧 리포트</span> 탭. <b>매주 월요일 오전</b>(기본 08:00, Asia/Seoul) 계정별 디스크·활동·실패 이력·특이사항을 이메일로 자동 발송합니다. 화면에서 이번 주 <b>미리보기</b>와 <b>지금 테스트 발송</b>이 가능합니다. 발송하려면 서버에 SMTP 환경변수를 설정해야 합니다(SMTP_HOST·SMTP_USER·SMTP_PASS·REPORT_TO 등). 미설정 시 미리보기는 되지만 발송은 비활성입니다.</li>
    ...
    <section class="manual-sec manual-tips">
      ...
      <ul>
        ...
        <li><b>파일형식 검증(항상 켜짐)</b> — 모든 업로드 파일의 매직바이트(실제 형식)를 확인해, 확장자만 바꾼 실행 파일(EXE/ELF/Mach-O 등)이나 내용이 확장자와 다른 파일을 자동 차단합니다. 별도 설정·서버 자원(데몬·메모리)이 필요 없습니다. 차단 시 감사 로그(file_blocked)에 사유가 남습니다.</li>
        <li><b>악성 패턴 검사 · YARA(로컬)</b> — 업로드 파일을 로컬 룰과 대조해 웹셸·난독화 스크립트·매크로 악성 등 <b>명백한 악성 패턴</b>을 차단합니다. <b>완전 오프라인</b>(파일·해시가 외부로 나가지 않음)이며 대시보드에서 켜짐/꺼짐을 확인할 수 있습니다. 차단 시 감사 로그(malware_blocked)에 사유가 남습니다.</li>
      </ul>
    </section>`;
