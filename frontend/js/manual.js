...
const Manual = (() => {
  ...
  const adminSections = `
    <section class="manual-sec">
      <h2>1. 대시보드</h2>
      <ul>
        <li><b>🏠 대시보드</b> 탭 — 계정/파일 수, 디스크 사용·할당 현황, 파일형식 검증(항상 켜짐)·바이러스 검사 상태를 한눈에 봅니다.</li>
    ...
    <section class="manual-sec manual-tips">
      ...
      <ul>
        ...
        <li><b>파일형식 검증(항상 켜짐)</b> — 모든 업로드 파일의 매직바이트(실제 형식)를 확인해, 확장자만 바꾼 실행 파일(EXE/ELF 등)이나 내용이 확장자와 다른 파일을 자동 차단합니다. 별도 설정·서버 자원이 필요 없습니다.</li>
        <li><b>바이러스 검사(선택)</b> — ClamAV(clamd)를 연결하면 업로드·업로드요청 파일을 시그니처 기반으로 추가 검사합니다(대시보드에서 상태 확인). 미연결 시에는 이 단계를 건너뛰며, 위 파일형식 검증은 그대로 동작합니다.</li>
      </ul>
    </section>`;
