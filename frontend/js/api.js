...
const API = (() => {
  ...
  return {
    ...
    // U10 잠금 · U6 이력 · U9 템플릿 · U12 도서정보
    lockFile: (id, note) => req('POST', `/files/${id}/lock`, { note: note || '' }),
    unlockFile: (id) => req('POST', `/files/${id}/unlock`),
    fileHistory: (id, limit) => req('GET', `/files/${id}/history` + (limit ? `?limit=${limit}` : '')),
    folderTemplates: (ownerId) => req('GET', '/files/templates' + (ownerId ? `?ownerId=${ownerId}` : '')),
    saveFolderTemplate: (data, ownerId) => req('POST', '/files/templates' + (ownerId ? `?ownerId=${ownerId}` : ''), data),
